...
- Use **11dB ADC attenuation (ADC_11db)** to extend the measurable range to ~3.3–3.6V; ~2.49V stays safely within it.
...
To run tests:
1. Set `TEST_MODE` to `true`.
2. Flash the board.
3. Open the Serial Monitor at 115200.

Set `TEST_MODE` back to `false` for normal operation. Tests print PASS/FAIL per case plus a summary; any FAIL means the firmware logic or constants need attention before deployment.
